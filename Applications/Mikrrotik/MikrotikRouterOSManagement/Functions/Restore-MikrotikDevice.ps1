function Restore-MikrotikDevice {
    <#
    .SYNOPSIS
        Restores a soft-deleted MikroTik device record in LiteDB.
    .DESCRIPTION
        Marks a previously deleted device inventory entry as active by
        RouterIdentityHostname in the mikrotik_devices collection.
    .PARAMETER RouterIdentityHostname
        Router identity/hostname to restore.
    .PARAMETER DatabasePath
        Full path to the LiteDB file. Defaults to Data\MikrotikDevices.db.
    .EXAMPLE
        PS> Restore-MikrotikDevice -RouterIdentityHostname 'R1'
    .INPUTS
        None.
    .OUTPUTS
        Boolean.
    ...
    try {
        ...
        if ($PSCmdlet.ShouldProcess("MikrotikDevice:$RouterIdentityHostname", 'Restore record')) {
        ...
        throw "Failed to restore MikroTik device record: $_"
    }
    finally {
        if ($null -ne $db) {
            $db.Dispose()
...
# Example usage:
# PS> Restore-MikrotikDevice -RouterIdentityHostname 'R1' -Confirm:$false
